...
API Key가 없어도 조회 화면 하단의 "예시 데이터로 보기" 버튼으로 대시보드와 검진 이력을 확인할 수 있습니다.
...
```
입력 폼 → 1차 인증 → 인증 대기(만료 카운트다운) → 2차 추가인증 → 대시보드
```
...
Mock 로그인과 사용자명 맞춤 문구, 그리고 API Key 없이 화면을 볼 수 있는 예시 데이터 모드를 제공합니다.
